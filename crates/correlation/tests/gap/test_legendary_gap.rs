...
fn gap_tls_weakness_should_group_by_port_not_just_host() {
    ...
    assert_eq!(chains.len(), 1, "GAP FINDING: TlsWeaknessRule fails to strip ports and correlate across the same host");
...
fn gap_correlation_engine_should_track_evidence_accurately() {
    ...
    assert!(evidence_str.contains(&findings[0].id().to_string()), "Should contain first finding ID");
    assert!(evidence_str.contains(&findings[1].id().to_string()), "GAP FINDING: Chain evidence is missing the second finding ID");
}
