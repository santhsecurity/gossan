...
async fn test_temporal_diffing_integration() {
    ...
    // First scan
    store.persist_scan(&[t1.clone()], &[]).expect("First scan failed");
    ...
    let diff = store.compute_diff(&[t1.clone(), t2.clone()], &[], std::time::Duration::from_secs(60)).expect("Diff failed");
    assert_eq!(diff.added_targets.len(), 1);
    assert_eq!(gossan_graph::target_id(&diff.added_targets[0]), "domain:new-domain.com");
    ...
    let diff = store.compute_diff(&[], &[], std::time::Duration::from_secs(3600)).expect("Diff failed");
    assert_eq!(diff.removed_targets.len(), 1, "Old last_seen should be considered removed");
