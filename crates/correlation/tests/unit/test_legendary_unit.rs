...
fn admin_exposed_rule_basic() {
    ...
    let findings = vec![
        create_finding("hidden", "admin.example.com", "Admin panel exposed", Severity::High),
        create_finding("hidden", "admin.example.com", "No authentication required", Severity::High),
    ];
    
    let new_findings = rule.check(&findings, &[]);
    assert_eq!(new_findings.len(), 1);
    assert_eq!(new_findings[0].severity(), Severity::Critical);
    assert!(new_findings[0].title().contains("Admin panel exposed without authentication"));
}
...
fn admin_exposed_rule_no_match() {
    ...
    let findings = vec![
        create_finding("hidden", "admin.example.com", "Admin panel exposed", Severity::High),
        create_finding("hidden", "other.example.com", "No authentication required", Severity::High),
    ];
...
fn tls_weakness_rule_basic() {
    ...
    let findings = vec![
        create_finding("portscan", "example.com", "Self-signed TLS certificate", Severity::Medium),
        create_finding("hidden", "example.com", "Missing HSTS header", Severity::Low),
    ];
