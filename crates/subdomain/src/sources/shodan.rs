...
impl SubdomainSource for Shodan {
    ...
    async fn query(
        ...
        let Some(_key) = crate::sources::get_api_key(config, "shodan", "SHODAN_API_KEY") else {
            return Ok(vec![]);
        };

        let url = format!("https://api.shodan.io/dns/domain/{domain}?key={key}", key = _key, domain = domain);
