...
async fn test_dependency_confusion_gap_incomplete_manifest_missing_heurstic() {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_string(
            r#"{ "version": "1.0.0", "devDependencies": { "@internal/tools": "1.0.0" } }"#,
        ))
        .mount(&server)
        .await;
    ...
    assert!(
        !findings.is_empty(),
        "Gap: Probe missed a package.json with devDependencies and no name/dependencies"
    );
}
...
async fn test_cors_gap_regex_origin_reflection() {
    ...
    assert!(
        !findings.is_empty(),
        "Gap: Probe does not check for suffix-based origin regex bypasses (example.com.evil.com)"
    );
}
