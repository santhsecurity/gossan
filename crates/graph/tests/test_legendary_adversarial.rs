...
fn test_adversarial_null_bytes() {
    ...
    assert!(format!("{err:?}").contains("null"), "rejection must name nulls: {err:?}");
    ...
    assert!(res.is_ok(), "store must accept Target with null-byte domain: {res:?}");
...
fn test_adversarial_huge_inputs() {
    ...
    assert!(format!("{err:?}").contains("Long"), "rejection must name length cap: {err:?}");
    ...
    assert!(res.is_ok(), "store must persist 1MB Target without panic: {res:?}");
...
fn test_adversarial_unicode_zalgo() {
    ...
    let diff = store.compute_diff(&[t], &[f], Duration::from_secs(10)).unwrap();
    assert_eq!(diff.added_targets.len(), 0);
}
...
fn test_adversarial_empty_inputs() {
    ...
    assert!(format!("{err:?}").contains("Empty"), "rejection must name emptiness: {err:?}");
    ...
    assert!(res.is_ok(), "store must persist empty-domain Target without panic: {res:?}");
