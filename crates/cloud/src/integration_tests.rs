...
<ListBucketResult xmlns="http://s3.amazonaws.com/doc/2006-03-01/">
    ...
    impl CloudProvider for TestS3Provider {
        ...
        async fn probe(&self, c: &reqwest::Client, n: &str, t: &Target) -> anyhow::Result<Vec<secfinding::Finding>> {
            ...
            if status == 200 && body.contains("<ListBucketResult") {
                findings.push(crate::finding_builder(t, secfinding::Severity::Critical, "Public Bucket", "Exposed").build().unwrap());
            }
            Ok(findings)
        }
    }
    ...
    let findings = provider.probe(&client, "test-bucket", &target).await.unwrap();
