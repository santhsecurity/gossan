...
//!
//! Re-homed (2026-05-17, C2) from the former standalone
//! `gossan-correlation` crate into the `gossan-graph` substrate it
//! always depended on  -  one coherent attack-surface-graph crate (model
//! + fusion), no longer split. `gossan-correlation` is kept one release as
//!   a thin re-export shim (upgrade-without-breaking-consumers).
//!
//! Crate-level lints are inherited from `gossan-graph`'s root.
//!
...
mod tests {
    ...
    fn engine_returns_api_auth_chain() {
        ...
        // Use an HTTP target + the canonical `auth-bypass` web tag  - 
        // the precision fix on 2026-05-22 stops chaining no-auth
        // findings from non-HTTP services (e.g. portscan DB banners).
