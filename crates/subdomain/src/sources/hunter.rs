...
impl SubdomainSource for Hunter {
    ...
    async fn query(
        ...
        let Some(_key) = crate::sources::get_api_key(config, "hunter", "HUNTER_API_KEY") else {
            return Ok(vec![]);
        };

        let url = format!("https://api.hunter.io/v2/domain-search?domain={domain}&api_key={key}", key = _key, domain = domain);
