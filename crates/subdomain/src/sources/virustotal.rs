...
impl SubdomainSource for Virustotal {
    ...
    async fn query(
        ...
        let Some(_key) = crate::sources::get_api_key(config, "virustotal", "VIRUSTOTAL_API_KEY") else {
            return Ok(vec![]);
        };

        let url = format!("https://www.virustotal.com/vtapi/v2/domain/report?apikey={key}&domain={domain}", key = _key, domain = domain);
