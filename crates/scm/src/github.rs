...
pub async fn discover_org_assets(domain: &str, config: &Config, input: &ScanInput) -> anyhow::Result<()> {
    ...
    match octo.orgs(org_name).get().await {
        Ok(org) => {
            ...
            loop {
                ...
                if repos.items.is_empty() { break; }
                
                for r in repos {
                    let repo_url_str = r.clone_url.map(|u| u.to_string()).unwrap_or_else(|| r.html_url.map(|u| u.to_string()).unwrap_or_default());
                    if repo_url_str.is_empty() { continue; }
