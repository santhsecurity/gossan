use gossan_core::ratelimit::{HostRateLimiter, build_client};
...
async fn test_host_rate_limiter_adversarial_huge() {
    ...
    assert!(elapsed < 1000, "Huge RPS should not delay requests significantly, elapsed: {}ms", elapsed);
}
...
async fn test_host_rate_limiter_adversarial_long_domain_string() {
    ...
    assert!(elapsed < 500, "Long domain strings must not break the rate limiter functionality");
}
