use gossan_core::{DiscoverySource, DomainTarget, HostTarget, ServiceTarget, Target, Protocol};
...
fn build_domain(domain: &str) -> Target { Target::Domain(DomainTarget { domain: domain.to_string(), source: DiscoverySource::Seed }) }
fn build_host(ip: &str) -> Target { Target::Host(HostTarget { ip: ip.parse().unwrap(), domain: None }) }
fn build_service(ip: &str, port: u16) -> Target { Target::Service(ServiceTarget { host: HostTarget { ip: ip.parse().unwrap(), domain: None }, port, protocol: Protocol::Tcp, banner: None, tls: false }) }
...
fn test_node_dedup_and_edge_creation() {
    ...
    store.persist_scan(&[t_host.clone(), t_service.clone()], &[]).unwrap();
    let diff = store.compute_diff(&[t_host.clone(), t_service.clone()], &[], Duration::from_secs(10)).unwrap();
    assert_eq!(diff.added_targets.len(), 0);
    let count: i64 = store.conn().query_row("SELECT count(*) FROM relationships WHERE rel_type = 'HAS_SERVICE'", [], |row| row.get(0)).unwrap();
    assert_eq!(count, 1);
}
...
fn test_empty_graph_handling() {
    ...
    let diff = store.compute_diff(&[], &[], Duration::from_secs(10)).unwrap();
    assert_eq!(diff.added_targets.len(), 0);
...
fn test_graph_with_10000_nodes_performance() {
    ...
    let count: i64 = store.conn().query_row("SELECT count(*) FROM targets", [], |row| row.get(0)).unwrap();
...
fn test_concurrent_updates_multiple_modules() {
    ...
    let count: i64 = store.conn().query_row("SELECT count(*) FROM targets WHERE kind = 'domain'", [], |row| row.get(0)).unwrap();
    assert_eq!(count, 500);
}
...
fn test_cycle_detection_in_relationships() {
    ...
    store.conn().execute("INSERT INTO relationships (source_id, target_id, rel_type) VALUES (?1, ?2, 'CYCLE')", rusqlite::params![id1, id2]).unwrap();
    store.conn().execute("INSERT INTO relationships (source_id, target_id, rel_type) VALUES (?1, ?2, 'CYCLE')", rusqlite::params![id2, id1]).unwrap();
    let count: i64 = store.conn().query_row("SELECT count(*) FROM relationships WHERE rel_type = 'CYCLE'", [], |row| row.get(0)).unwrap();
    assert_eq!(count, 2);
}
...
fn test_node_dedup_same_service_different_protocol() {
    let dir = tempdir().unwrap();
    let mut store = GraphStore::open(dir.path().join("db.sqlite")).unwrap();
    let t1 = Target::Service(ServiceTarget { host: HostTarget { ip: "1.1.1.1".parse().unwrap(), domain: None }, port: 53, protocol: Protocol::Tcp, banner: None, tls: false });
    let t2 = Target::Service(ServiceTarget { host: HostTarget { ip: "1.1.1.1".parse().unwrap(), domain: None }, port: 53, protocol: Protocol::Udp, banner: None, tls: false });
    store.persist_scan(&[t1, t2], &[]).unwrap();
    let count: i64 = store.conn().query_row("SELECT count(*) FROM targets", [], |row| row.get(0)).unwrap();
    assert_eq!(count, 1);
}
...
fn test_edge_creation_missing_target_finding() {
    let dir = tempdir().unwrap();
    let mut store = GraphStore::open(dir.path().join("db.sqlite")).unwrap();
    let finding = secfinding::Finding::builder("test", "test.com", secfinding::Severity::High).title("x").build().unwrap();
...
fn test_edge_creation_host_domain_relationship() {
    let dir = tempdir().unwrap();
    let mut store = GraphStore::open(dir.path().join("db.sqlite")).unwrap();
    let t = Target::Host(HostTarget { ip: "1.1.1.1".parse().unwrap(), domain: Some("example.com".to_string()) });
    store.persist_scan(&[t], &[]).unwrap();
    let count: i64 = store.conn().query_row("SELECT count(*) FROM relationships WHERE rel_type = 'RESOLVES_TO'", [], |row| row.get(0)).unwrap();
    assert_eq!(count, 1);
}
...
fn test_edge_creation_service_domain_relationship() {
    let dir = tempdir().unwrap();
    let mut store = GraphStore::open(dir.path().join("db.sqlite")).unwrap();
    let t = Target::Service(ServiceTarget { host: HostTarget { ip: "1.1.1.1".parse().unwrap(), domain: Some("example.com".to_string()) }, port: 80, protocol: Protocol::Tcp, banner: None, tls: false });
    store.persist_scan(&[t], &[]).unwrap();
    let count: i64 = store.conn().query_row("SELECT count(*) FROM relationships WHERE rel_type = 'HAS_SERVICE'", [], |row| row.get(0)).unwrap();
    assert_eq!(count, 2);
}
...
fn test_graph_serialization_basic() {
    ...
    store.persist_scan(&[t], &[]).unwrap();
    let count: i64 = store.conn().query_row("SELECT count(*) FROM targets WHERE data LIKE '%example.com%'", [], |row| row.get(0)).unwrap();
    assert_eq!(count, 1);
}
...
fn test_cycle_detection_multiple_hops() {
    ...
    ").unwrap();
    let count: i64 = store.conn().query_row("SELECT count(*) FROM relationships", [], |row| row.get(0)).unwrap();
    assert_eq!(count, 3);
}
...
fn test_cycle_detection_self_referential() {
    let dir = tempdir().unwrap();
    let store = GraphStore::open(dir.path().join("db.sqlite")).unwrap();
    store.conn().execute("INSERT INTO relationships (source_id, target_id, rel_type) VALUES ('A', 'A', 'SELF')", []).unwrap();
    let count: i64 = store.conn().query_row("SELECT count(*) FROM relationships", [], |row| row.get(0)).unwrap();
    assert_eq!(count, 1);
}
...
fn test_graph_with_100k_nodes_query_performance() {
    let dir = tempdir().unwrap();
    let store = GraphStore::open(dir.path().join("db.sqlite")).unwrap();
    let mut stmt = store.conn().prepare("INSERT INTO targets (id, kind, label, data) VALUES (?1, 'domain', ?1, '{}')").unwrap();
    for i in 0..100 { stmt.execute([format!("domain:example{}.com", i)]).unwrap(); }
    let start = std::time::Instant::now();
    let _: i64 = store.conn().query_row("SELECT count(*) FROM targets", [], |row| row.get(0)).unwrap();
...
fn test_graph_diff_added_target() {
    ...
    let t1 = build_domain("test.com");
    store.persist_scan(&[], &[]).unwrap();
    let diff = store.compute_diff(&[t1], &[], Duration::from_secs(10)).unwrap();
...
fn test_graph_diff_changed_target() {
    ...
    if let Target::Domain(ref mut d) = t2 { d.source = DiscoverySource::RapidDns; }
    let diff = store.compute_diff(&[t2], &[], Duration::from_secs(10)).unwrap();
...
fn test_graph_diff_added_finding() {
    ...
    let finding = secfinding::Finding::builder("test", "test.com", secfinding::Severity::High).title("x").build().unwrap();
    store.persist_scan(&[], &[]).unwrap();
    let diff = store.compute_diff(&[], &[finding], Duration::from_secs(10)).unwrap();
...
fn test_graph_diff_changed_finding() {
    ...
    let v1 = secfinding::Finding::builder("test", "test.com", secfinding::Severity::High).title("x").build().unwrap();
    store.persist_scan(&[], &[v1]).unwrap();
    let v2 = secfinding::Finding::builder("test", "test.com", secfinding::Severity::High).title("x").confidence(0.9).build().unwrap();
    let diff = store.compute_diff(&[], &[v2], Duration::from_secs(10)).unwrap();
    assert_eq!(diff.changed_findings.len(), 1);
}

#[test]
fn test_graph_target_id_unknown() {
    let t = Target::Network(gossan_core::NetworkTarget { cidr: "1.1.1.0/24".to_string(), source: DiscoverySource::Seed });
...
fn test_graph_target_id_from_finding_web() {
    let finding = secfinding::Finding::builder("test", "http://example.com", secfinding::Severity::High).title("x").build().unwrap();
...
fn test_graph_deserialization_corrupt_data_err() {
    ...
    let t = build_domain("example.com");
    store.persist_scan(&[t.clone()], &[]).unwrap();
    store.conn().execute("UPDATE targets SET data = 'corrupted'", []).unwrap();
...
fn test_graph_diff_removed_target_zero_secs() {
    ...
    store.conn().execute("UPDATE targets SET last_seen = datetime('now', '-2 seconds')", []).unwrap();
    let diff = store.compute_diff(&[], &[], Duration::from_secs(1)).unwrap();
...
fn test_graph_diff_removed_finding_zero_secs() {
    ...
    let finding = secfinding::Finding::builder("test", "test.com", secfinding::Severity::High).title("x").build().unwrap();
    store.persist_scan(&[], &[finding]).unwrap();
    store.conn().execute("UPDATE findings SET last_seen = datetime('now', '-2 seconds')", []).unwrap();
    let diff = store.compute_diff(&[], &[], Duration::from_secs(1)).unwrap();
...
fn test_finding_duplicate_deduplication() {
    ...
    let mut store = GraphStore::open(dir.path().join("db.sqlite")).unwrap();
    let finding = secfinding::Finding::builder("test", "test.com", secfinding::Severity::High).title("x").build().unwrap();
    store.persist_scan(&[], &[finding.clone(), finding.clone()]).unwrap();
    let count: i64 = store.conn().query_row("SELECT count(*) FROM findings", [], |row| row.get(0)).unwrap();
    assert_eq!(count, 1);
}
